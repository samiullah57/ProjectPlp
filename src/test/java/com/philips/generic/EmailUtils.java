...
public class EmailUtils {
	...
	public EmailUtils(String username, String password, String server, EmailFolder emailFolder)
			throws MessagingException {
		System.out.println("Prop conncetion");
		Properties props = System.getProperties();
		try {
			props.load(new FileInputStream(new File("src/test/java/com/philips/generic/configs/email.properties")));
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(-1);
		}
	...
	/**
	 * @param maxToGet
	 *            maximum number of messages to get, starting from the latest. For
	 *            example, enter 100 to get the last 100 messages received.
	 */
	 ...
	 * @param maxToSearch
	 *            maximum number of messages to search, starting from the latest.
	...
	public Message[] getMessagesBySubject(String subject, boolean unreadOnly, int maxToSearch) throws Exception {
        ...
        Message messages[] = {};
        int count =0;
        while(messages.length==0 && count<=5)
        {
	 ...
	 */
	public static List<String> getUrlsFromMessage(Message message, String linkText) throws Exception {
